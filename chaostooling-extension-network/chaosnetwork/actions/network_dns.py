...
def simulate_dns_timeout(
    ...
    with tracer.start_as_current_span("network.dns_timeout") as span:
        ...
        try:
            # Block DNS port
            subprocess.run([
                "iptables", "-A", iptables_chain, "-p", "udp", "--dport", str(dns_port), "-j", "DROP"
            ], check=True, capture_output=True)
            
            subprocess.run([
                "iptables", "-A", iptables_chain, "-p", "tcp", "--dport", str(dns_port), "-j", "DROP"
            ], check=True, capture_output=True)
            ...
            # Unblock
            subprocess.run([
                "iptables", "-D", iptables_chain, "-p", "udp", "--dport", str(dns_port), "-j", "DROP"
            ], check=False, capture_output=True)
            
            subprocess.run([
                "iptables", "-D", iptables_chain, "-p", "tcp", "--dport", str(dns_port), "-j", "DROP"
            ], check=False, capture_output=True)
