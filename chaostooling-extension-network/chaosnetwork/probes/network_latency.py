...
def probe_network_latency(
    ...
    try:
        with tracer.start_as_current_span("probe.network.latency") as span:
            ...
            logger.info(f"Network latency probe to {target_host}: avg={avg_latency}ms, loss={packet_loss}%")
            flush()
            return result_data
...
def probe_network_conditions() -> Dict:
    ...
    try:
        with tracer.start_as_current_span("probe.network.conditions") as span:
            ...
            logger.info(f"Network conditions: netem={has_netem}, latency={latency_ms}ms, loss={loss_percent}%")
            flush()
            return result_data
