...
def probe_network_connectivity(
    ...
    try:
        with tracer.start_as_current_span("probe.network.connectivity") as span:
            ...
            socket_type = socket.SOCK_STREAM if protocol.lower() == "tcp" else socket.SOCK_DGRAM
            ...
            if connected:
                ...
                logger.info(f"Network connectivity to {host}:{port} OK ({connection_time_ms:.2f}ms)")
            else:
                span.set_status(StatusCode.ERROR, error_msg or "Connection failed")
                logger.warning(f"Network connectivity to {host}:{port} failed: {error_msg}")
