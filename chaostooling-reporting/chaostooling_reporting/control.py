    ...
    **kwargs: Any,
    ...
    if not reporting_config:
        reporting_config = {
            ...
            "output_dir": os.getenv("CHAOS_REPORTING_OUTPUT_DIR", "/var/log/chaostoolkit/reports"),
            "formats": os.getenv("CHAOS_REPORTING_FORMATS", "html,json").split(","),
            "templates": {
                "executive": os.getenv("CHAOS_REPORTING_EXECUTIVE", "true").lower() == "true",
                "compliance": os.getenv("CHAOS_REPORTING_COMPLIANCE", "true").lower() == "true",
                "audit": os.getenv("CHAOS_REPORTING_AUDIT", "true").lower() == "true",
                "product_owner": os.getenv("CHAOS_REPORTING_PRODUCT_OWNER", "true").lower() == "true",
            },
        }

    # Initialize report generator
    output_dir = Path(reporting_config.get("output_dir", "/var/log/chaostoolkit/reports"))
