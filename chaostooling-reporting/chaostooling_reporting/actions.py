    ...
    try:
        ...
        if not journal_path:
            ...
            if env_journal_path and Path(env_journal_path).exists():
                ...
                logger.info(
                    f"Found journal from CHAOSTOOLKIT_JOURNAL_PATH: {journal_path}"
                )
            else:
                ...
                possible_paths.extend(
                    [
                        ...
                        / "journal.json",  # Explicit log directory
                    ]
                )
                ...
                possible_paths.extend(
                    [
                        Path("/experiments")
                        / "journal.json",  # Docker container experiments mount
                        Path("/experiments")
                        / "production-scale"
                        / "journal.json",  # Production scale experiments
                    ]
                )
        ...
        if not journal_path or not Path(journal_path).exists():
            ...
            logger.error(
                f"  - CHAOSTOOLKIT_JOURNAL_PATH: {os.getenv('CHAOSTOOLKIT_JOURNAL_PATH', 'not set')}"
            )
            logger.error(
                f"  - CHAOS_EXPERIMENT_DIR: {os.getenv('CHAOS_EXPERIMENT_DIR', 'not set')}"
            )
            logger.error(f"  - Current working directory: {Path.cwd()}")
            logger.error(
                "  - Primary log directory: /var/log/chaostoolkit/journal.json"
            )
            ...
            if log_dir.exists():
                try:
                    ...
                    logger.error(
                        f"  - Files in /var/log/chaostoolkit: {[f.name for f in files[:10]]}"
                    )
            ...
            try:
                ...
                logger.error(
                    f"  - Files in current directory: {[f.name for f in current_files[:10]]}"
                )
            except Exception as e:
        ...
        logger.info(
            f"Initializing report generator: output_dir={output_dir}, formats={formats_list}"
        )
        ...
        return {"success": False, "error": error_msg, "reports": {}}
