...
def get_db_connection():
    ...
    return psycopg2.connect(
        host=host.split(',')[0].split(':')[0] if ',' in host else host, # Take first host if list
    ...
    with tracer.start_as_current_span("mobile.observability.validate_data") as span:
        ...
        try:
            ...
            if count >= expected_count:
                ...
                logger.info(f"Data consistency check passed: Found {count} records (Expected >= {expected_count})")
                return True
            else:
                span.set_status(StatusCode.ERROR, "Data missing")
                logger.error(f"Data consistency check failed: Found {count} records (Expected >= {expected_count})")
                return False
