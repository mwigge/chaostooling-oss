...
def calculate_manual_risk_score(
    ...
    blast_radius = experiment_data.get("blast_radius", 5)  # 1-10 (how many services affected)
    recovery_difficulty = experiment_data.get("recovery_difficulty", 5)  # 1-10
    team_expertise = 10 - experiment_data.get("team_expertise_level", 5)  # Inverse (1-10)
    ...
    return {
        ...
        "calculation": {
            ...
            "step1_sum": (criticality * 0.25 + blast_radius * 0.25 + recovery_difficulty * 0.2 + team_expertise * 0.15 + (time_factor / 10) * 0.15),
...
def calculate_grafana_metrics_risk_score(
    ...
    error_score = min(error_rate / 10, 10)  # 0-10 (1% error = 0.1, 10% = 1.0, 100% = 10)
...
def analyze_risk_score_mismatch(
    ...
    if diff_percent > 50:
        ...
        if manual_score > grafana_score:
            ...
            explanation.append("• Possible: System has good redundancy/failover built in")
            explanation.append("• Possible: Conservative assessment (better safe than sorry)")
            explanation.append("• Possible: Pre-chaos metrics don't reflect chaos impact")
        else:
            explanation.append(
                f"• Grafana metrics show high risk ({grafana_score}), "
                f"but manual assessment is lower ({manual_score})"
            )
            explanation.append("• Possible: System already degraded before chaos experiment")
            explanation.append("• Possible: Metrics include unavoidable baseline spikes")
    ...
    if diff_percent > 50:
        ...
        recommendations.append("Validate Grafana metrics accuracy and datasource configuration")
        recommendations.append("Adjust risk weighting in either approach")
        recommendations.append("Monitor actual incident response time vs. estimated recovery difficulty")
    ...
    logger.info(f"Risk score analysis: Manual={manual_score}, Grafana={grafana_score}, Diff={diff:.2f}")
