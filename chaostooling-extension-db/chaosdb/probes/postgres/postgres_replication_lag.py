...
def check_replication_lag(
    ...
    if max_lag_seconds is not None:
        max_lag_seconds = float(max_lag_seconds) if isinstance(max_lag_seconds, str) else max_lag_seconds
    ...
    with span_context as span:
        ...
        try:
            ...
            if "name resolution" in error_msg.lower() or "could not translate host name" in error_msg.lower():
                logger.warning("Replication lag check: DNS resolution failed for %s (container may be stopped): %s", replica_host, e)
                ...
                if span:
                    span.record_exception(e)
                    span.set_status(StatusCode.ERROR, error_msg)
                logger.error(f"Replication lag check failed: {error_msg}", extra={"error": error_msg})
                flush()
                raise
