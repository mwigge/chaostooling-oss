...
def probe_replication_lag(
    ...
    if primary_port is not None:
        primary_port = int(primary_port) if isinstance(primary_port, str) else primary_port
    if replica_port is not None:
        replica_port = int(replica_port) if isinstance(replica_port, str) else replica_port
    
    primary_host = primary_host or os.getenv("POSTGRES_PRIMARY_HOST", "postgres-primary")
    primary_port = primary_port or int(os.getenv("POSTGRES_PRIMARY_PORT", "5432"))
    replica_host = replica_host or os.getenv("POSTGRES_REPLICA_HOST", "postgres-replica")
    ...
    with span_context as span:
        ...
        try:
            ...
            primary_conn = psycopg2.connect(
                host=primary_host, port=primary_port, database=database,
                user=user, password=password, connect_timeout=5
            )
            primary_cursor = primary_conn.cursor()
            primary_cursor.execute("SELECT pg_current_wal_lsn()")
            primary_lsn = primary_cursor.fetchone()[0]
            primary_cursor.execute("SELECT pg_wal_lsn_diff(pg_current_wal_lsn(), '0/0')")
            primary_lsn_numeric = primary_cursor.fetchone()[0]
            primary_cursor.close()
            primary_conn.close()
            
            # Get replica LSN
            replica_conn = psycopg2.connect(
                host=replica_host, port=replica_port, database=database,
                user=user, password=password, connect_timeout=5
            )
            ...
            if is_replica:
                ...
                receive_lag_seconds = (receive_lag_bytes / (16 * 1024 * 1024)) if receive_lag_bytes else None
                replay_lag_seconds = (replay_lag_bytes / (16 * 1024 * 1024)) if replay_lag_bytes else None
            else:
                receive_lag_bytes = None
                replay_lag_bytes = None
            ...
            if receive_lag_bytes is not None:
                if span:
                    span.set_attribute("replication.receive_lag_bytes", receive_lag_bytes)
                    span.set_attribute("replication.receive_lag_seconds", receive_lag_seconds or 0)
            if replay_lag_bytes is not None:
                if span:
                    span.set_attribute("replication.replay_lag_bytes", replay_lag_bytes)
                    span.set_attribute("replication.replay_lag_seconds", replay_lag_seconds or 0)
            ...
            if "name resolution" in error_msg.lower() or "could not translate host name" in error_msg.lower():
                logger.warning("Replication lag probe: DNS resolution failed for %s (container may be stopped): %s", replica_host, e)
                result = {
                    "success": False,
                ...
                if span:
                    span.record_exception(e)
                    span.set_status(StatusCode.ERROR, error_msg)
                logger.error(f"Replication lag probe failed: {error_msg}", extra={"error": error_msg})
                flush()
                raise
...
def probe_data_consistency(
    ...
    with span_context as span:
        ...
        try:
            ...
            # Handle DNS resolution errors gracefully (e.g., when container is stopped)
            if "name resolution" in error_msg.lower() or "could not translate host name" in error_msg.lower():
                logger.warning("Data consistency probe: DNS resolution failed for %s (container may be stopped): %s", replica_host, e)
                result = {
                    "success": False,
                    "is_consistent": False,
                    "error": "DNS resolution failed - container may be stopped",
                ...
                if span:
                    span.record_exception(e)
                    span.set_status(StatusCode.ERROR, error_msg)
                logger.error(f"Data consistency probe failed: {error_msg}", extra={"error": error_msg})
                flush()
                raise
