    ...
    with span_context as span:
        try:
            ...
            if span:
                ...
                span.set_attribute("chaos.activity", "mongodb_document_contention_status")
            ...
            # Get write conflicts
            write_conflicts = server_status.get("wiredTiger", {}).get("concurrentTransactions", {}).get("write", {}).get("conflicts", 0)
            ...
            metrics.record_db_error(db_system="mongodb", error_type=type(e).__name__, db_name=database)
            if span:
                span.record_exception(e)
                span.set_status(StatusCode.ERROR, str(e))
            logger.error(f"MongoDB document contention probe failed: {str(e)}", extra={"error": str(e)})
            flush()
            raise
