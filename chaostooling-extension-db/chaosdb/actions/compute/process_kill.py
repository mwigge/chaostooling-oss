...
def kill_process_by_name(
    ...
    try:
        with tracer.start_as_current_span("chaos.compute.process_kill") as span:
            ...
            logger.info(
                f"Killing processes matching '{process_name}' with {signal_type}"
            )
            ...
            for proc in matching_processes:
                try:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"Process {pid} did not terminate, force killing"
                        )
                        proc.kill()
                        proc.wait(timeout=5)

                    killed_processes.append(
                        {"pid": pid, "name": proc_name, "signal": signal_type}
                    )
                    ...
                    metrics.record_db_error(
                        db_system=db_system, error_type="AccessDenied"
                    )
            ...
            span.set_status(
                StatusCode.OK if len(killed_processes) > 0 else StatusCode.ERROR
            )
        ...
        metrics.record_db_error(
            db_system=db_system, error_type=type(e).__name__, process_name=process_name
        )
...
def kill_process_by_pid(
    ...
    try:
        ...
        db_system = os.getenv("DB_SYSTEM", "postgresql")
        metrics.record_db_error(
            db_system=db_system, error_type=type(e).__name__, pid=str(pid)
        )
        logger.error(f"Process kill failed: {e}")
        flush()
        raise
