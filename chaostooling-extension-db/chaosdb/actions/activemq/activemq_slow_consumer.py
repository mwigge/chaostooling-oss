...
class SlowConsumerListener(stomp.ConnectionListener):
    def __init__(
        self, consumer_id, consume_delay_ms, tracer, logger, host, port, queue
    ):
    ...
    def on_message(self, frame):
        with self.tracer.start_as_current_span(
            f"slow_consumer.message.{self.consumer_id}"
        ) as span:
            ...
            try:
                ...
                tags = get_metric_tags(
                    db_system="activemq", db_operation="slow_consume"
                )
                ...
                self.logger.error(
                    f"Slow consumer {self.consumer_id} message processing error: {e}"
                )
...
def inject_slow_consumer(
    ...
    def slow_consumer_worker(consumer_id: int):
        ...
        try:
            with tracer.start_as_current_span(
                ...
                listener = SlowConsumerListener(
                    consumer_id, consume_delay_ms, tracer, logger, host, port, queue
                )
                conn.set_listener("", listener)
                conn.connect(user, password, wait=True)
                conn.subscribe(
                    destination=f"/queue/{queue}", id=consumer_id, ack="client"
                )
    ...
    try:
        with tracer.start_as_current_span("chaos.activemq.slow_consumer") as span:
            ...
            logger.info(
                f"Starting ActiveMQ slow consumers with {num_consumers} consumers for {duration_seconds}s"
            )
