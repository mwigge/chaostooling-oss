...
def inject_dlq_saturation(
    ...
    dlq_queue = dlq_queue or os.getenv("ACTIVEMQ_DLQ_QUEUE", f"{queue}.dlq")
            ...
            span.set_attribute("chaos.operation", "dlq_saturation")
                ...
                while not _stop_event.is_set() and time.time() < end_time and message_count < messages_per_producer:
                    try:
                        ...
                        conn.send(destination=f"/queue/{dlq_queue}", body=message)
    ...
    try:
        with tracer.start_as_current_span("chaos.activemq.dlq_saturation") as span:
            span.set_attribute("messaging.system", "activemq")
            span.set_attribute("messaging.destination", dlq_queue)
            ...
            logger.info(f"Starting ActiveMQ DLQ saturation with {num_producers} producers for {duration_seconds}s")
            ...
            logger.info(f"ActiveMQ DLQ saturation completed: {result}")
            flush()
            return result
    except Exception as e:
        _stop_event.set()
