...
def inject_rebalancing_storm(
    ...
    consumer_group = consumer_group or os.getenv(
        "KAFKA_CONSUMER_GROUP", "chaos_rebalancing_group"
    )
    ...
    try:
        with tracer.start_as_current_span("chaos.kafka.rebalancing_storm") as span:
            ...
            logger.info(
                f"Starting Kafka rebalancing storm with {num_consumers} consumers for {duration_seconds}s"
            )

            threads = []
            for i in range(num_consumers):
                thread = threading.Thread(
                    target=consumer_worker, args=(i,), daemon=True
                )
                thread.start()
