...
def inject_query_saturation(
    ...
    queries_per_thread = int(queries_per_thread) if isinstance(queries_per_thread, str) else queries_per_thread
    duration_seconds = int(duration_seconds) if isinstance(duration_seconds, str) else duration_seconds
    slow_query_threshold_ms = int(slow_query_threshold_ms) if isinstance(slow_query_threshold_ms, str) else slow_query_threshold_ms
    ...
    def query_worker(thread_id: int):
        ...
        try:
            ...
            error_msg = str(e)
            is_connection_refused = "Connection refused" in error_msg or "Access denied" in error_msg
            connection_failure_type = "connection_refused" if is_connection_refused else "operational_error"
