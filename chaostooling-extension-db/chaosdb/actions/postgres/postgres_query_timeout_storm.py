...
def inject_query_timeout_storm(
                ...
                while not _stop_event.is_set() and time.time() < end_time:
                    try:
                        ...
                        try:
                            ...
                            tags = get_metric_tags(db_name=database, db_system="postgresql", db_operation="timeout")
                            metrics.record_db_query_timeout(db_system=db_system, db_name=database, count=1)
                        ...
                        tags = get_metric_tags(db_name=database, db_system="postgresql", db_operation="query")
    ...
    try:
        with tracer.start_as_current_span("chaos.postgres.query_timeout_storm") as span:
            ...
            logger.info(f"Starting PostgreSQL query timeout storm with {num_threads} threads for {duration_seconds}s")
