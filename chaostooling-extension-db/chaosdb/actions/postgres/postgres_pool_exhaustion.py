...
def inject_connection_pool_exhaustion(
    ...
    num_connections = int(num_connections) if isinstance(num_connections, str) else num_connections
    ...
    def create_and_hold_connection(conn_id: int):
        ...
        try:
            with tracer.start_as_current_span(f"pool_exhaustion.connection.{conn_id}") as span:
                ...
                span.set_attribute("chaos.activity", "postgresql_connection_pool_exhaustion")
                ...
                try:
                    ...
                    metrics.record_db_histogram("connection.acquisition_time", acquisition_time_ms, db_system=db_system, db_name=database, unit="ms")
                    metrics.record_db_counter("connection.pool.active", db_system=db_system, db_name=database, count=1)
                    ...
                    metrics = get_metrics_core()
                    metrics.record_db_histogram("connection.wait_time", wait_time_ms, db_system=db_system, db_name=database, unit="ms")
                    metrics.record_db_error(db_system=db_system, error_type="OperationalError", db_name=database)
            ...
            if conn and not leak_connections:
                ...
                metrics = get_metrics_core()
                metrics.record_db_counter("connection.leak", db_system=db_system, db_name=database, count=1)
                logger.warning(f"Leaking connection {conn_id} (intentional)")
    
    try:
        with tracer.start_as_current_span("chaos.postgres.connection_pool_exhaustion") as span:
            span.set_attribute("db.system", "postgresql")
            span.set_attribute("db.name", database)
            ...
            logger.info(f"Starting connection pool exhaustion with {num_connections} connections")
                ...
                for conn in _active_connections:
                    try:
                        ...
                        if False:  # Placeholder - pool tracking
                            connection_pool_active_counter.add(-1, get_metric_tags(
