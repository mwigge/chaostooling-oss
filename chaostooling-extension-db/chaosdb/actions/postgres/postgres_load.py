...
def force_sequential_scans(
    ...
    def run_scans():
        ...
        try:
            ...
            while not stop_event.is_set():
                ...
                cursor.execute(
                    f"SELECT count(*) FROM {table_name} WHERE created_at::text LIKE '%202%';"
                )
                conn.commit()
                ...
                metrics.record_db_counter(
                    "seq_scans", db_system=db_system, db_name=database, count=1
                )
    ...
    try:
        with tracer.start_as_current_span(
            ...
            logger.info(
                f"Starting sequential scans on {table_name} for {duration_seconds}s with {num_threads} threads"
            )
    ...
    try:
        with tracer.start_as_current_span(
            ...
            for i in range(count):
                ...
                cursor.execute(
                    f"UPDATE {table_name} SET amount = amount + 0.01 WHERE id = (SELECT id FROM {table_name} LIMIT 1 OFFSET {i % 100});"
                )

            # Re-enable autovacuum
            try:
                cursor.execute(
                    f"ALTER TABLE {table_name} SET (autovacuum_enabled = true);"
                )
            except Exception as e:
        ...
        try:
            ...
            while not stop_event.is_set():
                ...
                metrics.record_db_gauge(
                    "temp_files", 1, db_system=db_system, db_name=database
                )
