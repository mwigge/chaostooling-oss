"""PostgreSQL slow transaction chaos action."""
import os
import psycopg2
import time
import threading
...
def inject_slow_transactions(
    ...
    transaction_delay_ms = int(transaction_delay_ms) if isinstance(transaction_delay_ms, str) else transaction_delay_ms
    ...
    tracer = get_tracer()
    logger = logging.getLogger("chaosdb.postgres.slow_transactions")
    ...
    def slow_transaction_worker(thread_id: int):
        ...
        try:
            with tracer.start_as_current_span(f"slow_transaction.worker.{thread_id}") as span:
                ...
                span.set_attribute("chaos.activity", "postgresql_slow_transactions")
                ...
                while not _stop_event.is_set() and time.time() < end_time:
                    try:
                        ...
                        # Lock a row
                        cursor.execute(f"SELECT * FROM {table_name} WHERE id = 1 FOR UPDATE")
                        cursor.fetchone()
                        ...
                        if txn_duration_ms > transaction_delay_ms:
                            span.set_attribute(
                                "chaos.slow_transaction_detected", True
                            )
    ...
    try:
        with tracer.start_as_current_span("chaos.postgres.slow_transactions") as span:
            ...
            logger.info(f"Starting slow transactions with {num_threads} threads for {duration_seconds}s")
...
def _prepare_table(host, port, database, user, password, table_name, logger):
    ...
    try:
        ...
        """)
        # Ensure updated_at column exists even if table was created with an older schema
        cursor.execute(
