...
def inject_connection_exhaustion(
    ...
    def create_and_hold_connection(conn_id: int):
        ...
        try:
            with tracer.start_as_current_span(f"connection_exhaustion.connection.{conn_id}") as span:
                ...
                span.set_attribute("chaos.activity", "mongodb_connection_exhaustion")
                ...
                try:
                    client = MongoClient(uri, maxPoolSize=1, serverSelectionTimeoutMS=5000)
    ...
    try:
        with tracer.start_as_current_span("chaos.mongodb.connection_exhaustion") as span:
            span.set_attribute("db.system", db_system)
            span.set_attribute("db.name", database)
            ...
            logger.info(f"Starting connection exhaustion with {num_connections} connections")
