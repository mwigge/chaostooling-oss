...
def inject_deadlock(
            ...
            with tracer.start_as_current_span(
                ...
                while not _stop_event.is_set() and time.time() < end_time:
                    try:
                        ...
                        # Lock first resource
                        cursor.execute(
                            f"SELECT * FROM {table_name} WITH (UPDLOCK, ROWLOCK) WHERE id = ?",
                            first_id,
                        )
                        cursor.fetchone()
                        ...
                        cursor.execute(
                            f"UPDATE {table_name} SET value = value + 1 WHERE id = ?",
                            first_id,
                        )
                        cursor.execute(
                            f"UPDATE {table_name} SET value = value + 1 WHERE id = ?",
    ...
    try:
        with tracer.start_as_current_span("chaos.mssql.deadlock_injection") as span:
            ...
            logger.info(
                f"Starting MSSQL deadlock injection with {num_threads} threads for {duration_seconds}s"
            )
