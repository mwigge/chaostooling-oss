...
def test_mssql_connection(
    ...
    try:
        with tracer.start_as_current_span("test.mssql.connection") as span:
            ...
            tags = get_metric_tags(db_name=database, db_system="mssql", db_operation="connectivity_test")
            if metrics_module.operation_duration_histogram:
                metrics_module.operation_duration_histogram.record(connection_time_ms, tags)
            metrics.record_db_query_count(db_system=db_system, db_name=database, count=1)
            span.set_status(StatusCode.OK)
            logger.info(f"MSSQL connection OK: {connection_time_ms:.2f}ms", extra={"connection_time_ms": connection_time_ms})
            flush()
            return dict(success=True, connection_time_ms=connection_time_ms, database=database, host=host)
    except Exception as e:
        span.set_status(StatusCode.ERROR, str(e))
        if metrics_module.error_counter:
            metrics_module.error_counter.add(1, get_metric_tags(db_name=database, error_type=type(e).__name__))
