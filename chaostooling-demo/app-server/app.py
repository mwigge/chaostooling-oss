...
def purchase():
    ...
    try:
        resp = requests.post(f"{inventory_service_url}/check", json={"item_id": item_id, "quantity": 1}, timeout=5)
        if resp.status_code != 200:
    ...
    return jsonify({"status": "success", "message": "Purchase completed", "transaction_id": transaction_id, "order_id": order_id}), 200
