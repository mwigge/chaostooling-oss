...
def collect_baseline_snapshot(
    ...
    try:
        ...
        for metric in metrics:
            ...
            try:
                ...
                snapshot["metrics"][metric] = {
                    "query": metric,
                    "result": result.get("results", [{}])[0] if result.get("results") else result.get("data", {}),
        ...
        logger.info(f"  Collected {len([m for m in snapshot['metrics'].values() if m.get('status') == 'success'])}/{len(metrics)} metrics")
...
def collect_trace_snapshot(
    ...
    try:
        ...
        trace_snapshot = {
            ...
            "datasource": datasource_uid,
            "query": query,
            "traces": result.get("results", [{}])[0].get("frames", []) if result.get("results") else [],
            "status": result.get("status", "error")
        }
...
def collect_log_snapshot(
    ...
    try:
        ...
        log_snapshot = {
            ...
            "datasource": datasource_uid,
            "query": query,
            "logs": result.get("results", [{}])[0].get("frames", []) if result.get("results") else [],
            "status": result.get("status", "error")
        }
