...
KNOWN_SERVICES = {
    # Databases
    "postgres", "postgresql", "mysql", "mssql", "sqlserver", "mongodb", "mongo",
    "cassandra", "redis", "dynamodb", "elasticsearch", "couchdb", "neo4j",
    
    # Messaging & Streaming
    "rabbitmq", "kafka", "activemq", "sqs", "sns", "kinesis", "pubsub",
    
    # Host-based
    "host", "cpu", "memory", "disk", "io", "filesystem",
    
    # Network-based
    "network", "latency", "bandwidth", "packet", "dns", "tcp", "udp",
    
    # Container/Orchestration
    "docker", "kubernetes", "k8s", "ecs", "fargate",
    
    # Cloud Services
    "aws", "azure", "gcp", "ec2", "lambda", "s3"
}
...
def before_experiment_control(context: Dict[str, Any], 
                             state: Any = None,
    ...
    try:
        ...
        context["chaos_experiment_title"] = experiment.get("title", "Unknown Experiment")
    ...
    **kwargs: Any
    ...
    try:
        ...
        if isinstance(state, dict):
            ...
            deviated = not steady_state_hypothesis.get("steady_state_met", False) if steady_state_hypothesis else None
        else:
        ...
        service_name = context.get("chaos_service_name", "unknown") if context else "unknown"
        title = context.get("chaos_experiment_title", "Unknown") if context else "Unknown"
        ...
        if deviated is not None:
            logger.info(f"Steady State: {'FAILED - Deviation Detected' if deviated else 'PASSED'}")
        
        # Show where results are stored
        logger.info("")
        logger.info("Results stored in chaos_platform database:")
        logger.info(f"  - View summary: SELECT * FROM chaos_platform.experiment_runs_summary WHERE run_id = {run_id};")
        logger.info(f"  - View audit: SELECT * FROM chaos_platform.experiment_audit_trail WHERE run_id = {run_id};")
