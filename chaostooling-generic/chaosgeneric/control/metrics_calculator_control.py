            ...
            if "metrics" in arguments:
                ...
                if isinstance(metrics_list, list):
                    for m in metrics_list:
                        metrics.append({
                            "metric_name": m if isinstance(m, str) else m.get("name", "unknown"),
                            "metric_query": m if isinstance(m, str) else m.get("query", ""),
                            "criticality": "high"
                        })
    ...
    def save_experiment_metrics(
        ...
        try:
            ...
            run_record = self.db.execute_fetch_one(
                "SELECT id FROM experiment_runs WHERE run_id = %s",
                (run_id,)
            )
...
def before_experiment_start(context: Dict[str, Any], **kwargs) -> None:
    ...
    try:
        ...
        if not experiment:
            logger.debug(f"No experiment in context/kwargs. Context keys: {context.keys() if context else 'None'}")
        ...
        logger.info(f"Calculating metrics for experiment: {experiment.get('title', 'unknown')}")
        ...
        test_score = calculator.calculate_test_score([m["metric_name"] for m in metrics])
