...
def on_experiment_start(context: Dict[str, Any] = None, **kwargs) -> None:
    ...
    try:
        ...
        if "method" in context:
            for action in context.get("method", []):
                if "baseline" in action.get("name", "").lower():
                    service_name = action.get("provider", {}).get("arguments", {}).get("service_name", service_name)
                    baseline_file = action.get("provider", {}).get("arguments", {}).get("output_file", baseline_file)
        ...
        logger.warning("✗ No baseline data found - collecting baseline before experiment...")
    ...
    try:
        ...
        with db._get_connection() as conn:
            with conn.cursor() as cur:
                ...
                if count > 0:
                    logger.debug(f"✓ Found {count} baseline metrics in database for {service_name}")
                    return True
