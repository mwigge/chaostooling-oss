...
def query_prometheus_metric(
    metric_name: str, prometheus_url: str = "http://prometheus:9090"
) -> Optional[float]:
    """
    ...
    try:
        response = requests.get(
            f"{prometheus_url}/api/v1/query", params={"query": metric_name}, timeout=5
        )
...
def before_experiment_control(
    ...
    **kwargs,
    ...
    try:
        ...
        title = context.get("chaos_experiment_title") or experiment.get(
        ...
        if not experiment_id:
            ...
            logger.warning(
                "Add 'experiment-orchestrator' as the FIRST control in your experiment"
            )
...
def after_experiment_control(
    ...
    **kwargs,
    ...
    try:
        ...
        steady_state_passed = (
            journal.get("steady_state_hypothesis", {}).get("passed", False)
            if journal
            else False
        )
        ...
        if calculate_risk_level and calculate_complexity_score:
            try:
                ...
                risk_level = risk_calc.get(
                    "level_name", "Medium"
                ).lower()  # low, medium, high, critical
                risk_score = int(
                    risk_calc.get("score", 50) * 2.55
                ...
                complexity_score = int(
                    complexity_calc.get("score", 50) * 2.55
                )  # Scale 0-100 to 0-255
        ...
        logger.info(
            f"Steady-State Hypothesis: {'PASSED' if steady_state_passed else 'FAILED'}"
        )
        logger.info(f"Findings: {findings_count}")
        logger.info(
            f"Success Rate: {success_rate}%" if success_rate else "Success Rate: N/A"
        )
        logger.info(
            f"Risk Score: {risk_score if risk_score is not None else 'N/A'}/255 (calculated by chaosotel.calculator)"
        )
        logger.info(
            f"Complexity Score: {complexity_score if complexity_score is not None else 'N/A'}/255 (calculated by chaosotel.calculator)"
        )
        logger.info(
            f"Test Quality: {test_quality if test_quality is not None else 'N/A'}/255 (based on steady-state, findings, success rate)"
        )
        logger.info(
            f"Risk Level: {risk_level.upper() if risk_level else 'N/A'} (derived from risk score)"
        )
        logger.info("=" * 80)
