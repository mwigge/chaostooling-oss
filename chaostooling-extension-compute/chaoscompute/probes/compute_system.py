...
from chaosotel import ensure_initialized, get_metric_tags, flush, get_metrics_core
...
def get_container_stats(container_name: str, logical_name: str | None = None) -> Dict[str, float]:
    """
    ...
    try:
        ...
        cmd = ["docker", "stats", container_name, "--no-stream", "--format", "{{.CPUPerc}},{{.MemPerc}}"]
        ...
        logger.debug(f"Recorded stats for {container_name}: CPU={cpu_percent}%, Mem={mem_percent}%")
        flush()
...
def collect_container_metrics(containers: list = None) -> Dict[str, Any]:
    ...
    for target in target_names:
        ...
        if found:
            ...
            logger.warning(f"Container matching '{target}' not found in {running_containers}")
