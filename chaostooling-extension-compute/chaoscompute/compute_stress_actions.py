...
class LinuxComputeStress:
    ...
    def cpu_stress(
        ...
        try:
            ...
            cmd = [
                "stress-ng",
                "--cpu", str(workers),
                "--cpu-ops", "0",  # Run indefinitely
                "--timeout", f"{duration_seconds}s",
                "--metrics",
                "--quiet"
            ]
            
            logger.info(f"Starting CPU stress: {workers} workers, {duration_seconds}s duration")
    ...
    def memory_stress(
        ...
        try:
            ...
            cmd = [
                "stress-ng",
                "--vm", str(workers),
                "--vm-bytes", f"{memory_to_stress_mb}M",
                "--vm-ops", "0",  # Run indefinitely
                "--timeout", f"{duration_seconds}s",
                "--metrics",
                "--quiet"
            ]
            
            logger.info(f"Starting memory stress: {memory_to_stress_mb}MB, {duration_seconds}s duration")
    ...
    def disk_io_stress(
        ...
        try:
            ...
            logger.info(f"Starting disk I/O stress: {workers} workers, {duration_seconds}s duration in {directory}")
    ...
    def filesystem_stress(
        ...
        try:
            ...
            logger.info(f"Starting filesystem stress: {workers} workers, {duration_seconds}s duration")
...
class WindowsComputeStress:
    ...
    def disk_io_stress(
        ...
        try:
            ...
            logger.info(f"Starting disk I/O stress: {duration_seconds}s duration in {directory}")
...
def stress_cpu(
    ...
    """
    logger.info(f"[COMPUTE] Stressing CPU: {workers or 'all'} workers for {duration_seconds}s")
...
def stress_memory(
    ...
    """
    logger.info(f"[COMPUTE] Stressing memory: {memory_percent}% for {duration_seconds}s")
    stress = get_compute_stress()
    result = stress.memory_stress(workers, memory_percent, duration_seconds)
    logger.info(f"[COMPUTE] Memory stress completed: {result.get('exit_code', 'unknown')}")
    return result
...
def stress_disk_io(
    ...
    logger.info(f"[COMPUTE] Disk I/O stress completed: {result.get('exit_code', 'unknown')}")
    return result
...
def stress_filesystem(
    ...
    """
    logger.info(f"[COMPUTE] Stressing filesystem: {duration_seconds}s, {file_size_mb}MB files")
    stress = get_compute_stress()
    result = stress.stress_filesystem(workers, duration_seconds, directory, file_size_mb)
    logger.info(f"[COMPUTE] Filesystem stress completed: {result.get('exit_code', 'unknown')}")
    return result
